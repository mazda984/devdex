...
/*
  Lightweight local JSON persistence using localStorage.
  Provides a minimal API compatible with the previous code:
    room.collection('game').getList()
    room.collection('game').create(record)
    room.collection('game').subscribe(callback)
*/
const STORAGE_KEY = 'devdex_games_v1';
...
function _writeStorage(list){
  try{
    ...
    // notify subscribers with newest->oldest (to match previous behavior)
    const copied = list.slice().sort((a,b)=> b.created_at.localeCompare(a.created_at));
    const records = copied.map(r=> ({ ...r })); // shallow copy
    _subscribers.forEach(cb => {
      try{ cb(records); }catch(e){ console.warn('subscriber error', e); }
    });
  }catch(e){
...
const room = {
  collection(name){
    if(name !== 'game'){
      // generic fallback: still use same key
      console.warn('Using JSON storage for collection:', name);
    }
    return {
      async getList(){
        // Return newest->oldest to match WebsimSocket behavior
      ...
      async create(record){
        ...
        const rec = { ...record, id, created_at: now, username: record.author || record.username || 'anon', type: 'game' };
        items.push(rec);
        _writeStorage(items);
        return rec;
      },
      // subscribe provides a callback that is immediately invoked with current list and called on changes
      subscribe(cb){
        if(typeof cb !== 'function') return () => {};
        _subscribers.add(cb);
        // call immediately with current list (newest->oldest)
        const list = _readStorage().slice().sort((a,b)=> b.created_at.localeCompare(a.created_at));
        const records = list.map(r=> ({ ...r }));
        try{ cb(records); }catch(e){ console.warn('subscriber callback error', e); }
